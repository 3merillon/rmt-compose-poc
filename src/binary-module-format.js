...
function deserializeNote(data, view, offset) {
  ...
  for (let i = 0; i < varCount; i++) {
    ...
    // Extract dependency info from bytecode
    const deps = [];
  ...
  const color = colorLen > 0 ? decoder.decode(data.slice(offset, offset + colorLen)) : null;
  offset += colorLen;

  // Instrument
  const instLen = data[offset++];
  const instrument = instLen > 0 ? decoder.decode(data.slice(offset, offset + instLen)) : null;
  offset += instLen;
