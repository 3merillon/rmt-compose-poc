...
- [x] **6.4b Melodies + scale systems** — generator `scripts/gen-melodies.mjs`: **7 relational public-domain melodies** (Ode to Joy, Twinkle, Frère Jacques, Amazing Grace, Greensleeves, Bach Minuet in G, Scarborough Fair; each note `(scale-degree)*base.f`, timing from `base.t` → transposes/re-roots on drop) verified against the canonical tunes by a 7-agent workflow. Moved TET-12/19/31 + BP-13 + Mixed-Base into a new **Scale Systems** section, added **Tesla 9-EDO**, dropped the redundant rational `12.json`, unshipped `test`/`test3`, removed the old melodies `V7-I`/`V-I` (now in Progressions/Cadences).
