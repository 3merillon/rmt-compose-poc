...
**Browser baselines** (headless ANGLE/swiftshader, `?perf=1`, p50 ms) — recorded during Phase 8. `sync` = `renderer.sync()` in isolation; `commit` = end-to-end `player:octaveChange` (rewrite + eval + sync + history). "Before" = post-Phase-1 (P2-P5); "After" = post-Phase-8 (corruption-loop fix + ring batch + history dedupe):

| Scenario (≈1000 notes) | sync before → after | commit before → after |
|---|---|---|
| chain-1000 (depth 1000) | **28.5 → 1.4** (20×) | **45.8 → 17.6** (2.6×) |
| lattice-1000 (depth ~100) | 18.9 → 1.8 | 34.0 → 15.3 |
| chords-dense (depth ~4) | 5.6 → 1.1 | 9.1 → 4.7 |
| fan-1000 (depth 1) | 1.4 → 1.2 | 2.2 → 1.9 |
| default (170) | 0.7 → 0.5 | 0.8 → 0.6 |

`sync` was O(N²) in dependency depth (the corruption-flag scan); it is now flat ~1–1.4ms at 1000 notes regardless of depth. `full eval` was already ~1ms (Phase 1) and is unchanged. Selecting a 999-dependent hub: ring-pass redraw draws collapsed 999→6 (chain redraw p50 13.6→9.5ms; the win is larger on real GPUs where fill is cheap and draw-call overhead dominates).
...
- [x] `src/settings/settings-panel.js` — **floating, draggable, non-modal widget** (same species as the note-variables widget: `z-index` 1200, dragged by its header via the shared `src/utils/draggable-widget.js`, stays open while you compose). Opened from the top-bar gear (`#settingsGearBtn`) or the "Settings…" menu entry; tabs **Appearance | Arrows | Audio | Library**; live write-through; per-tab + global reset, each behind a confirmation (`src/utils/confirm-dialog.js`). Subscribes to `settingsStore` while open so it can't go stale. Self-contained CSS using `var(--rmt-*)` with orange fallbacks. Same floating card on mobile — draggable by touch, not a full-screen sheet. Verified in the real browser by `scripts/perf/shot-settings.mjs` (48 checks).
...
## Phase 8 — Performance Round 2   `[x]`*   Last touched: 2026-07-12 by Claude

**Measurement-first. Profiled in-browser (headless ANGLE/swiftshader, `?perf=1`) at 1000 notes across all four stress modules BEFORE touching code — this reframed the phase.** `[x]`* = the core wins shipped + verified; P7 idle-gating is **deferred** (regression, see below).

**Key finding that reframed the batching item:** the renderer is already far more batched than the original bullet implied. Every draw path that scales with N — note bodies, silence rings, fraction dividers, arrow backgrounds, tabs, measure bars, measure triangles, octave guides, **dependency link-lines**, and all overlay text — is **already a single instanced draw**. Re-touching them is pure pixel-regression risk for zero saving. The profiling also showed **the real per-commit hotspot was not draw calls but an O(N²) loop inside `renderer.sync()`.**

- [x] **The real win — kill the O(N²) corruption-flag scan in `sync()`** (renderer.js ~736). The old path called `getAllDependencies()` (a full transitive BFS) for *every* note *every* sync, so cost scaled with dependency **depth**: at 1000 notes `sync` was **28.5ms (chain, depth 1000)** vs **1.4ms (fan, depth 1)** — identical note counts. Replaced with ONE multi-source BFS over the *dependents* graph seeded by the (usually empty) corrupt set: **O(N+E)**, and a no-op when nothing is corrupt (the common case). Provably identical output (5600-trial randomized differential test vs the old per-note algo, **0 mismatches**). Result: `sync` now **flat ~1–1.4ms** regardless of depth (chain **28.5→1.4ms, 20×**; lattice **18.9→1.8ms**; chords **5.6→1.1ms**); end-to-end octave commit **chain 45.8→17.6ms (2.6×)**, lattice 34.0→15.3ms, chords 9.1→4.7ms. Pixel-identical (GL framebuffer signature).
- [x] **Batch the dependency-highlight ring pass** (renderer.js `drawIdxList` ~2355) — the ONE remaining per-instance loop that can approach O(N) (only when a highly-connected note is selected, e.g. a fan hub with ~1000 dependents). When no drag is active (the redraw-during-hover/playhead worst case), `u_dragOffset` is `(0,0)` for every ring so the whole bucket collapses to one instanced draw: **999→6 draws + 999→6 `bufferData` uploads per selection**. The ring shader reads only `a_posSize` per instance and WebGL rasterizes instances in order, so it is pixel-identical; the drag path keeps the exact per-instance loop. Verified never-slower (chain redraw 13.6→9.5ms) and pixel-identical. **The "measure-bars / dep-lines" clauses of the original bullet were already satisfied** — marked done rather than re-implemented. Measure-triangle outline batching intentionally skipped (bounded by measure count M≪N, not a bottleneck).
- [x] **Cheap `sync()` win:** reuse a cached zero-buffer for the per-sync region-buffer resets instead of allocating a fresh `Float32Array` every sync (removes GC churn during drag; identical GPU behavior). Descoped the measure-buffer / noteIdToIndex micro-gates — profiling showed sync already met the frame budget after the corruption fix, and correct gating would have been zoom/scale-dependent (pixel-risk) for sub-0.3ms yield.
- [x] **Rollup `manualChunks`** — split the 653KB entry monolith into cacheable leaves: `renderer` (251KB, verified singleton-free), `vendor` (fraction.js), `dsl` (+ binary-note/binary-utils to break the core↔dsl cycle), `instruments`. **Entry 653→390KB.** Duplication-free (grep-verified: GLSL only in the renderer chunk, core singletons only in the entry). Production build boots identically (all 8 chunks 200, notes render, octave round-trips, **0 console errors**).
- [x] **Cheap history (deferred P8, safe form)** — store undo/redo snapshots as **minified JSON strings** instead of parsed object graphs (each restore parses fresh → same isolation, ~3-5× less retained heap), **dedupe** the previous 2-3× per-action serialize (capture + localStorage now share one `snapshotStr`), and add a 12MB byte cap. Restore stays the proven `loadFromJSON` path (full reverse-patch was **not** pursued — poor risk/reward: it doesn't reduce the dominant per-undo recompile+eval cost and adds the most delicate correctness surface in the app). Verified: 9/9 undo/redo round-trip (octave up×2 → undo×2 → redo×2 → new-edit-clears-redo), 0 errors.
- [!] **P7 idle-rAF-gating — DEFERRED (caused a visible regression).** The GL loop is already `needsRedraw`-gated; the app never idles only because `player.js` re-dirties it every frame (`setPlayhead` + a per-frame `camera.onChange()`). Guarding those made idle GPU work drop to ~0 (verified: 0 draws over 2s idle, playback/selection still redraw) — **but it exposed a latent bug: the note-background overlay passes (tab/arrow region buffers on shared attrib 4) are not idempotent within a single `_render()`.** The old continuous 60fps redraw silently guaranteed a per-frame `_viewEpoch` bump that rebuilt+re-uploaded those region buffers every frame; with idle-gating the backgrounds flickered/vanished until the next redraw (mouse move). **Reverted** (pixel-identical to golden restored). **Fix path for a future session:** make the tab/arrow/divider overlay passes frame-idempotent (re-point attrib 4 to their own region buffer before each draw, independent of the epoch gate) so a single `_render()` fully paints; *then* the idle-gating (setPlayhead change-guard + drop the per-frame `camera.onChange`) is safe. Lowest-value Phase 8 item (roadmap already noted "battery upside is small"), so deferring is the disciplined call.
- [x] Re-ran harness; Phase 0 baseline table updated with the browser before/after (below).

**Deferred with sharper rationale (unchanged from Phase 1):** full stable-slot incremental `sync()` (the O(N) fill/upload is not the bottleneck — the O(N²) scan was, now fixed) and full reverse-patch history remain deferred; P7 loop-gating now additionally blocked on the overlay non-idempotency above.

**Dev tooling added (perf-gated, production-inert):** `?perf` now also exposes `window.__rmtRenderer/__rmtWorkspace` and the harness gained `measureSync`/`measureRedraw`/`emit`/`noteFreq`/`pickFreqNoteId` for the browser before/after measurements and undo/redo tests.
...
- **2026-07-12** — **Phase 8 mostly COMPLETE** (Performance Round 2), measurement-first, verified in headless Chromium. **Profiled at 1000 notes before coding**, which reframed the phase: the renderer is already fully instanced (note bodies, rings, dividers, arrows, tabs, measure bars/triangles, octave guides, dep-lines, overlay text — all single instanced draws), and the real per-commit hotspot was an **O(N²) corruption-flag scan in `renderer.sync()`** (`getAllDependencies` per note per sync → cost scaled with dependency depth: 28.5ms chain vs 1.4ms fan at identical 1000-note counts). Fixed with one O(N+E) multi-source dependents-BFS from the corrupt set (no-op when nothing is corrupt), **proven identical by a 5600-trial randomized differential test (0 mismatches)** → `sync` now flat ~1–1.4ms regardless of depth (chain **28.5→1.4ms, 20×**), octave commit **chain 45.8→17.6ms (2.6×)**. **Batched the one remaining per-N per-instance loop** (dependency-highlight rings, `drawIdxList`): a selection with 999 dependents collapses **999→6 draws + uploads** when not dragging (drag path unchanged); pixel-identical, never slower. **Reused a cached zero-buffer** for per-sync region resets (GC churn). **Rollup `manualChunks`**: entry monolith **653→390KB** + cacheable `renderer`(251KB)/`vendor`/`dsl`/`instruments` leaves, duplication-free (grep-verified), production build boots identically (0 errors). **Cheap history**: undo/redo snapshots stored as **minified JSON strings** (not object graphs; ~3-5× less heap, each restore parses fresh) with the previous 2-3× per-action serialize **deduped** and a 12MB cap; restore stays the proven `loadFromJSON` path (full reverse-patch declined — poor risk/reward); 9/9 undo/redo round-trip verified. **Every renderer change verified pixel-identical** via a deterministic GL-framebuffer signature over 10 scenes. **P7 idle-rAF-gating DEFERRED**: it worked (idle GPU → 0 draws) but exposed a latent bug — the tab/arrow note-background overlay passes aren't idempotent within one `_render()` (the old per-frame `_viewEpoch` bump masked it), so idle-gating made backgrounds flicker/vanish until a mouse move; reverted (pixel-identical restored), with the fix path (make the overlay passes frame-idempotent first) documented in the Phase 8 section. Perf harness gained browser-side `measureSync`/`measureRedraw`/`emit`/`noteFreq` (perf-gated, production-inert).
